...
int main(int argc, char **argv) {
    ...
    if (!rank)
        printf("MPI All-to-All benchmark\n");
    ...
    // Linear exchange algorithm (commented out for simplicity)
    // ...

    // Torsten Hoefler's algorithm
    MPI_Barrier(MPI_COMM_WORLD);
    if (!rank)
        printf("All-to-All Torsten Hoefler's algorithm\n");
    ...
    for (i = 0; i < size; i++) {
        peer = (((rank % 2) + (i % 2)) % 2) * (size / 2) + ((rank / 2) + (i / 2)) % (size / 2);
